...
# Physical Tag0 mounting correction.
#
# Converts Tag0's PnP local frame → world frame.
# Depends ONLY on how Tag0 is physically mounted — NOT on camera orientation.
# Tag0 is on a VERTICAL surface, Y axis = physical UP, Z axis = out of face (toward camera).
#
#   World X =  Tag0 X   (horizontal right, parallel to Tag1-Tag2 bar)
#   World Y = -Tag0 Z   (depth into scene, away from tag face)
#   World Z =  Tag0 Y   (physical vertical UP)
#
# If the world Z appears inverted (Pink below base), swap sign of row 2.
# If X is mirrored, swap sign of row 0.
R_TAG0_CORRECTION = np.array([
    [1,  0,  0],   # X_world =  X_Tag0
    [0,  0, -1],   # Y_world = -Z_Tag0
    [0,  1,  0],   # Z_world =  Y_Tag0  (UP)
], dtype=float)
...
def detect_other_tags(img, intr, base_T, base_rvec, corners, ids, depth_image=None, depth_scale=1.0):
    ...
    if 1 in tag_positions and 2 in tag_positions:
        ...
        # 1. Get Tag0 rotation from PnP (camera → Tag0 local frame)
        rmat_tag0, _ = cv2.Rodrigues(base_rvec)

        if USE_TAG0_YZ_TO_XY_REMAP:
            rmat_tag0 = rmat_tag0 @ R_YZ_TO_XY

        # 2. Build rmat_inv: Tag0-frame → world frame.
        # R_TAG0_CORRECTION converts Tag0's local axes to world axes based on
        # Tag0's PHYSICAL MOUNTING only — independent of camera orientation.
        # rmat_tag0.T maps from camera frame to Tag0 local frame (PnP inverse).
        rmat_inv = R_TAG0_CORRECTION @ rmat_tag0.T
    ...
    try:
        ...
        while True:
            ...
            if T_base is not None:
                ...
                rmat_tag0, _ = cv2.Rodrigues(rvec_use)
                if USE_TAG0_YZ_TO_XY_REMAP:
                    rmat_tag0 = rmat_tag0 @ R_YZ_TO_XY
                # Camera-orientation-independent world frame:
                # R_TAG0_CORRECTION encodes Tag0's physical mounting only.
                rmat_inv = R_TAG0_CORRECTION @ rmat_tag0.T
